...
public class TestClassUtils {
  ...
  @Test public void testNewInstanceOfWithStringArguments() {
    assertEquals(Arrays.asList(new String[]{"a"}),
                 ClassUtils.newInstanceOf("org.wikimedia.diffdb.TestClassUtils$MyCollection(a)", Arrays.asList(new String[]{"fail"}),
                                          Collection.class));
    assertEquals(Arrays.asList(new String[]{"xy"}),
                 ClassUtils.newInstanceOf("org.wikimedia.diffdb.TestClassUtils$MyCollection(x,y)", Arrays.asList(new String[]{"fail"}),
                                          Collection.class));
    assertEquals(Arrays.asList(new String[]{"fail"}),
                 ClassUtils.newInstanceOf("org.wikimedia.diffdb.TestClassUtils$MyCollectionX(a)", Arrays.asList(new String[]{"fail"}),
                                          Collection.class));
  }

  @Test public void testNewInstanceOfWithIntegerArguments() {
    // assertEquals(Arrays.asList(new String[]{"i6"}),
    //              ClassUtils.newInstanceOf("org.wikimedia.diffdb.TestClassUtils$MyCollection(1,2,3)", Arrays.asList(new String[]{"fail"}),
    //                                       Collection.class));
  }
